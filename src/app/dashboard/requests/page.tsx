...
import {
  RequestDetailsSheet,
  type RequestSheetData,
} from '@/components/dashboard/requests/RequestDetailsSheet';
...
import { createGoogleCalendarEvent } from '@/lib/googleCalendar/createGoogleCalendarEvent';
import { deleteGoogleCalendarEvent } from '@/lib/googleCalendar/deleteGoogleCalendarEvent';
...
export default function AllRequestsPage() {
  ...
  const [activeTechnicianIdFilter, setActiveTechnicianIdFilter] = useState<
    string | null
  >(null);

  const [isSheetOpen, setIsSheetOpen] = useState(false);
  const [selectedRequestForSheet, setSelectedRequestForSheet] =
    useState<RequestSheetData | null>(null);
  ...
  useEffect(() => {
    ...
    if (statusFilterFromUrl) {
      const statuses = statusFilterFromUrl
        .split(',')
    ...
    if (technicianIdFilterFromUrl) {
      setActiveTechnicianIdFilter(
        decodeURIComponent(technicianIdFilterFromUrl)
      );
    } else {
      ...
      try {
        ...
        const fetchedRequests = querySnapshot.docs.map((docSnap) => {
          ...
          return {
            ...
            completata_da_collaboratore_nome:
              data.completata_da_collaboratore_nome,
            data_completamento: data.data_completamento as
              | Timestamp
              | undefined,
        ...
        toast({
          ...
          description: "Impossibile caricare l'elenco delle richieste.",
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  const filteredRequests = useMemo(() => {
    ...
    return allRequests.filter((req) => {
      const matchesSearch =
        ...
        (req.email_cliente &&
          req.email_cliente.toLowerCase().includes(searchTermLower)) ||
        req.service.toLowerCase().includes(searchTermLower) ||
        (req.assegnato_a_tecnico_nome &&
          req.assegnato_a_tecnico_nome.toLowerCase().includes(searchTermLower));
      ...
      const matchesTechnician =
        !activeTechnicianIdFilter ||
        req.assegnato_a_tecnico_id === activeTechnicianIdFilter;
  ...
  const handleUpdateRequestStatusOnPage = async (
    requestId: string,
    newStatus: string,
    ...
    try {
      const requestRef = doc(db, 'richieste_clienti', requestId);
      await updateDoc(requestRef, { stato: newStatus, ...additionalData });

      toast({
        title: 'Successo!',
        description: `Stato della richiesta aggiornato a "${newStatus}".`,
      });
      ...
      // Получаем accessToken из azienda
      const aziendaSnap = await getDoc(doc(db, 'aziende', companyId));
      const azienda = aziendaSnap.data();
      const accessToken = azienda?.google_auth?.google_access_token;
      const isConnected = azienda?.google_auth?.google_calendar_connected;
      const calendarId = 'primary'; // заменишь на ID кастомного календаря, если понадобится

      if (!isConnected || !accessToken) return;

      // Получаем richiesta
      const richiestaSnap = await getDoc(requestRef);
      const richiestaData = richiestaSnap.data();
      const existingEventId = richiestaData?.google_calendar_event_id;

      if (['programmata', 'in corso'].includes(newStatus)) {
        if (!existingEventId) {
          const eventId = await createGoogleCalendarEvent(
            { ...richiestaData, id: requestId },
            calendarId,
            accessToken
          );
          ...
          toast({
            title: 'Sincronizzato!',
            description: 'Evento aggiunto nel calendario Google.',
          });
        }
      } else if (['completata', 'annullata'].includes(newStatus)) {
        if (existingEventId) {
          ...
          console.log('🗑 Событие удалено из календаря');
          toast({
      ...
      toast({
        title: 'Errore',
        description: 'Errore durante aggiornamento o sincronizzazione.',
        variant: 'destructive',
      });
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Visualizza, filtra e gestisci tutte le richieste di intervento.
          </p>
        </div>
        <Button
          asChild
        ...
        <CardHeader>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="ml-auto shrink-0">
                  <Filter className="mr-2 h-4 w-4" /> Filtra per Stato (
                  {activeStatusFilters.length > 0
          ...
          {activeTechnicianIdFilter && technicians.length > 0 && (
            <div className="mt-2 text-sm text-muted-foreground">
              Filtro tecnico attivo:{' '}
              <Badge variant="secondary">
                {technicians.find((t) => t.id === activeTechnicianIdFilter)
                  ?.nome_completo || activeTechnicianIdFilter}
              </Badge>
            </div>
          )}
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      <th className="p-3 text-left text-sm font-semibold text-muted-foreground sticky top-0 bg-card z-10">
                        ID Richiesta
                      </th>
                      <th className="p-3 text-left text-sm font-semibold text-muted-foreground sticky top-0 bg-card z-10">
                        Cliente
                      ...
                      <th className="p-3 text-right text-sm font-semibold text-muted-foreground sticky top-0 bg-card z-10">
                        Azioni
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredRequests.map((req) => (
                      <tr key={req.id} className="border-b hover:bg-muted/50">
                        <td className="p-3 text-sm font-medium text-primary whitespace-nowrap">
                          ...
                          <Badge
                            ...
                            className={
                              req.status === 'completata'
                                ...
                                : req.status === 'assegnata'
                                ? 'bg-blue-100 text-blue-700 border-blue-200'
                                : req.status === 'programmata'
                        ...
                        <td className="p-3 text-right text-sm whitespace-nowrap">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleOpenDetailsSheet(req)}
                          >
                            Dettagli
                          </Button>
                        </td>
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">
                Prova a modificare i filtri o aggiungi una nuova richiesta.
              </p>
            </div>
          )}
        </CardContent>
