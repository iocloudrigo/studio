...
const unifiedRegisterFormSchema = z
  .object({
    adminName: z
      .string()
      .min(2, { message: 'Il Tuo Nome e Cognome è richiesto.' }),
  ...
  .refine(
    (data) => {
      ...
      if (!auth.currentUser) {
        // A more direct check if we are in a new user registration scenario
...
export function UnifiedRegisterForm({
  ...
  const onSubmit = async (data: UnifiedRegisterFormValues) => {
    ...
    let userDisplayNameToUse: string | undefined =
      prefilledUser?.displayName || data.adminName;

    if (!prefilledUser) {
      console.log(
        '[UnifiedRegisterForm] Attempting new user registration with email:',
        data.email
      );
      if (!data.password || data.password.length < 6) {
        form.setError('password', {
          message: 'La password è richiesta e deve avere almeno 6 caratteri.',
        });
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        await updateProfile(userCredential.user, {
          displayName: userDisplayNameToUse,
        });
        console.log(
          '[UnifiedRegisterForm] New user created successfully with Firebase Auth:',
          userIdToUse,
          'DisplayName set to:',
          userDisplayNameToUse
        );
      } catch (authError: any) {
        console.error(
          '[UnifiedRegisterForm] Errore creazione utente Firebase Auth:',
          authError
        );
        ...
        if (authError.code === 'auth/email-already-in-use')
          ...
          errorMessage = `Errore Auth: ${
            authError.message || 'Errore sconosciuto'
          }`;
        toast({
      ...
      if (data.adminName !== prefilledUser.displayName && auth.currentUser) {
        try {
          ...
          console.log(
            '[UnifiedRegisterForm] DisplayName aggiornato per utente pre-autenticato:',
            data.adminName
          );
        } catch (updateError) {
          console.error(
            '[UnifiedRegisterForm] Errore aggiornamento displayName per utente pre-autenticato:',
            updateError
          );
        }
      }
    ...
    if (!userIdToUse || !userEmailToUse) {
      console.error(
        '[UnifiedRegisterForm] User ID or Email is undefined after auth step.'
      );
      toast({
        title: 'Errore Interno',
        description: 'Impossibile ottenere i dati utente.',
        variant: 'destructive',
      });
      setIsLoading(false);
      return;
    ...
    console.log(
      '[UnifiedRegisterForm] User ID for Firestore:',
      userIdToUse,
      'User Email:',
      userEmailToUse,
      'User Display Name:',
    ...
    const validationResult = unifiedRegisterFormSchema.safeParse({
      ...data,
      slug: finalSlug,
    });
    if (!validationResult.success) {
      console.error(
        '[UnifiedRegisterForm] Form validation failed after slug normalization:',
    ...
    console.log(
      '[UnifiedRegisterForm] Form data validated successfully after slug normalization.'
    );

    try {
      console.log(
        '[UnifiedRegisterForm] Querying Firestore for slug uniqueness:',
        finalSlug
      );
      ...
      if (slugTakenByOther) {
        console.warn(
          '[UnifiedRegisterForm] Slug conflict detected. Slug:',
          finalSlug,
          'is taken by another company.'
        );
        form.setError('slug', {
          type: 'manual',
          message:
            "Questo slug è già utilizzato da un'altra azienda. Scegline uno diverso.",
        });
        toast({
          title: 'Errore Registrazione',
          description: 'Lo slug scelto è già in uso.',
          variant: 'destructive',
        });
        setIsLoading(false);
        return;
      ...
      const companyDataToSave = {
        ...
        settore_attivita:
          data.activitySector === 'unspecified' || !data.activitySector
            ? null
            : data.activitySector,
      ...
      console.log(
        '[UnifiedRegisterForm] Attempting to save company data to Firestore:',
        JSON.stringify(companyDataToSave, null, 2)
      );
      const companyDocRef = doc(db, 'aziende', userIdToUse);
      await setDoc(companyDocRef, companyDataToSave);
      console.log(
        '[UnifiedRegisterForm] Company data saved successfully to Firestore for document ID:',
        userIdToUse
      );
      ...
      await addDoc(
        ...
        adminCollaboratorData
      );
      console.log(
        '[UnifiedRegisterForm] Admin collaborator entry created for:',
        userEmailToUse,
        'with name:',
        data.adminName
      );
      ...
      toast({
        ...
        description:
          'La tua azienda è stata configurata. Verrai reindirizzato alla dashboard.',
      });
      router.push('/dashboard');
      console.log('[UnifiedRegisterForm] Redirecting to /dashboard.');
    } catch (error: any) {
      console.error(
        '[UnifiedRegisterForm] Errore generico in onSubmit:',
        error
      );
      toast({
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input
                          ...
                          disabled={
                            isLoading ||
                            (!!prefilledUser && !!prefilledUser.displayName)
                          }
                        />
                      </div>
                    </FormControl>
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (
                            !isSlugManuallyEdited &&
                            e.target.value !== generateSlug(companyNameValue)
                          ) {
                            setIsSlugManuallyEdited(true);
                          }
